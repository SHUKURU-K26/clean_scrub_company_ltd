...
export default function StockOut() {
  ...
  const bulkColumns = ['Product', 'Quantity', 'Customer', 'Phone', 'Type', 'Value', 'Date'];
  const bulkRows = () => selectedRows.map((t) => [t.productName, `-${t.quantity}`, t.customerName, t.customerPhone, t.customerType, formatCurrency(t.quantity * t.unitPrice), formatDateTime(t.date)]);

  const handleBulkExportPdf = () => exportToPdf({ title: 'Stock Out — Selected Entries', columns: bulkColumns, rows: bulkRows(), filename: 'stock-out-selected' });
  const handleBulkExportExcel = () => exportToExcel({ title: 'Stock Out', columns: bulkColumns, rows: bulkRows(), filename: 'stock-out-selected' });
  ...
  return (
    <div className="space-y-5">
      ...
      <StockOutFormModal open={formOpen} onClose={() => setFormOpen(false)} transaction={editingTx} />
